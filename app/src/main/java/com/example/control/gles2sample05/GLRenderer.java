package com.example.control.gles2sample05;
...
public class GLRenderer implements GLSurfaceView.Renderer {
    ...
    public void onDrawFrame(GL10 glUnused) {
        ...
        //MySphereの描画（茶色のラグビーボール）
        Matrix.setIdentityM(mMatrix, 0);
        Matrix.rotateM(mMatrix, 0, 1.5f * angle, 0, 1, 0);
        Matrix.translateM(mMatrix, 0, -0.8f, 0f, 0f);
        Matrix.rotateM(mMatrix, 0, 1.5f * angle, 0, 1, 0);
        Matrix.scaleM(mMatrix, 0, 0.1f, 0.1f, 0.2f);
        GLES.updateMatrix(mMatrix);//現在の変換行列をシェーダに指定
        //MySphereの描画本体
        // r, g, b, a, shininess(1以上の値　大きな値ほど鋭くなる)
        MySphere.draw(1f, .7f, .7f, 1f, 5.f);

        //MySphereの描画（紫色の丸座布団）
        Matrix.setIdentityM(mMatrix, 0);
        Matrix.rotateM(mMatrix, 0, 1.5f * angle, 0, 1, 0);
        Matrix.translateM(mMatrix, 0, 0f, 0f, 0.8f);
        Matrix.rotateM(mMatrix, 0, -1.5f * angle, 0, 0, 1);
        Matrix.scaleM(mMatrix, 0, 0.25f, 0.1f, 0.25f);
        GLES.updateMatrix(mMatrix);//現在の変換行列をシェーダに指定
        //MySphereの描画本体
        // r, g, b, a, shininess(1以上の値　大きな値ほど鋭くなる)
        MySphere.draw(0.8f, .2f, 1f, 1f, 5.f);
